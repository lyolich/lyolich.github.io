(function () {
    ...
    function parseYouTube(html) {
        ...
        try {
            var json = html.split("var ytInitialData =")[1].split(";</script>")[0];
            var data = JSON.parse(json);

            var items = data.contents.twoColumnSearchResultsRenderer.primaryContents.sectionListRenderer.contents;

            items.forEach(function (section) {
                if (!section.itemSectionRenderer) return;

                section.itemSectionRenderer.contents.forEach(function (video) {
                    if (!video.videoRenderer) return;

                    var v = video.videoRenderer;
