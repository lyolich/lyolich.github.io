...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  window.rch_nws[hostkey].typeInvoke('http://z01.online', function() {
    ...
    client.on("RchClient", function(rchId, url, data, headers, returnHeaders) {
      ...
      if (url == 'eval') {
        ...
        console.warn('RCH eval request blocked'); result('eval disabled');
      } else if (url == 'ping') {
        result('pong');
      } else {
        console.log('RCH', url);
  ...
  // Safe stub: do not load remote websocket client or external scripts.
  try {
    if (typeof call === 'function') call();
  } catch (e) {
    console.warn('rchRun callback error', e);
    try { if (typeof call === 'function') call(); } catch(e2) {}
  }
  ...
  Object.keys(file.qualitys).forEach(function(key) {
    ...
    // Safer implementation: prefer user setting if available, otherwise choose highest available quality.
    try {
      var keys = Lampa.Arrays.getKeys(data.quality || {});
      var zqual = Lampa.Storage.field('video_quality_default');

      if (keys.length && zqual) {
        // try to find exact match for user's preference
        for (var qi = 0; qi < keys.length; qi++) {
          var qk = keys[qi];
          if (parseInt(qk, 10) == parseInt(zqual, 10)) {
            if (data.quality[qk]) {
              data.url = data.quality[qk];
      ...
      if (keys.length) {
        // pick highest numeric quality key available
        var numeric = keys.map(function(k){ return parseInt(k,10) || 0; }).sort(function(a,b){return b-a;});
        for (var ii = 0; ii < numeric.length; ii++) {
          var mk = numeric[ii];
          if (data.quality[mk]) {
            data.url = data.quality[mk];
            this.orUrlReserve(data);
            return;
          }
        }
      ...
      console.warn('setDefaultQuality error', e);
    }
