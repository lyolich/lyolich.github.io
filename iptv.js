/*
// IPTV Plugin - List View Edition (Modern UI - Full & Working with Forced Logo Box)
// Based on MaxTV Plugin
*/
;(function () {
'use strict';
console.log('[max_iptv_list] build v3-logo-cascade-debug');
var plugin = {
    component: 'max_iptv_list',
...
function trName(word) {
    return word.split('').map(function (char) {
	return trW[char] || char;
    }).join("");
}
function chShortName(chName) {
    return chName
	.toLowerCase()
	.replace(/\s+\((\+\d+)\)/g, ' $1')
	.replace(/^телеканал\s+/, '')
...
/* ---- Real channel-logo database (iptv-org), used as priority source before abstract fallbacks ---- */
var logoMap = null;
var logoNetwork = new Lampa.Reguest();
var LOGO_DB_TTL = 7 * 24 * 3600 * 1000; // 7 days
var LOGO_DB_KEY = plugin.component + '_logo_db_v1';
var LOGO_CH_URL = 'https://iptv-org.github.io/api/channels.json';
var LOGO_LOGOS_URL = 'https://iptv-org.github.io/api/logos.json';

function loadLogoDatabase(callback) {
    if (logoMap) { callback(); return; }
    var cached = null;
    try { cached = JSON.parse(localStorage.getItem(LOGO_DB_KEY) || 'null'); } catch (e) { cached = null; }
    if (cached && cached.ts && (Date.now() - cached.ts) < LOGO_DB_TTL && cached.map) {
	logoMap = cached.map;
	callback();
	return;
    }
    var channelsData = null, logosData = null, need = 2;
    var done = function () {
	if (--need) return;
	if (!channelsData || !logosData) {
	    console.warn('[max_iptv_list] logo DB fetch failed (channels:' + (!!channelsData) + ' logos:' + (!!logosData) + ') - iptv-org source unreachable, skipping it');
	    logoMap = (cached && cached.map) || {};
	    callback();
	    return;
	}
	var idToUrl = {};
	for (var i = 0; i < logosData.length; i++) {
	    var l = logosData[i];
	    if (!l || !l.channel || !l.url) continue;
	    if (!idToUrl[l.channel] || l.in_use) idToUrl[l.channel] = l.url;
	}
	var map = {};
	for (var j = 0; j < channelsData.length; j++) {
	    var c = channelsData[j];
	    if (!c || !c.id) continue;
	    var url = idToUrl[c.id];
	    if (!url) continue;
	    var names = [c.name].concat(c.alt_names || []);
	    for (var n = 0; n < names.length; n++) {
		if (!names[n]) continue;
		var key = chShortName(names[n]);
		if (key && !map[key]) map[key] = url;
	    }
	}
	logoMap = map;
	console.log('[max_iptv_list] logo DB built: ' + Object.keys(map).length + ' names indexed');
	try { localStorage.setItem(LOGO_DB_KEY, JSON.stringify({ts: Date.now(), map: map})); } catch (e) {}
	callback();
    };
    logoNetwork.silent(LOGO_CH_URL, function (d) { channelsData = d; done(); }, function (e) { console.warn('[max_iptv_list] channels.json fetch failed', e); done(); });
    logoNetwork.silent(LOGO_LOGOS_URL, function (d) { logosData = d; done(); }, function (e) { console.warn('[max_iptv_list] logos.json fetch failed', e); done(); });
}

Lampa.Template.add(plugin.component + '_style', '<style>#PLUGIN_epg{margin-right:1em}.PLUGIN-program__desc{font-size:0.9em;margin:0.5em;text-align:justify;max-height:15em;overflow:hidden;}.PLUGIN.category-full{padding-bottom:10em}#PLUGIN{float:right;padding: 1.2em 0;width: 30%;}.PLUGIN-details__group{font-size:1.3em;margin-bottom:.9em;opacity:.5}.PLUGIN-details__title{font-size:4em;font-weight:700}.PLUGIN-details__program{padding-top:4em}.PLUGIN-details__program-title{font-size:1.2em;padding-left:4.9em;margin-top:1em;margin-bottom:1em;opacity:.5}.PLUGIN-details__program-list>div+div{margin-top:1em}.PLUGIN-details__program>div+div{margin-top:2em}.PLUGIN-program{display:-webkit-box;display:-webkit-flex;display:-moz-box;display:-ms-flexbox;display:flex;font-size:1.2em;font-weight:300}.PLUGIN-program__time{-webkit-flex-shrink:0;-ms-flex-negative:0;flex-shrink:0;width:5em;position:relative}.PLUGIN-program.focus .PLUGIN-program__time::after{content:\'\';position:absolute;top:.5em;right:.9em;width:.4em;background-color:#ffd700;height:.4em;-webkit-border-radius:100%;-moz-border-radius:100%;border-radius:100%;margin-top:-0.1em;font-size:1.2em}.PLUGIN-program__progressbar{width:10em;height:0.3em;border:0.05em solid #ffd700;border-radius:0.05em;margin:0.5em 0.5em 0 0}.PLUGIN-program__progress{height:0.25em;border:0.05em solid #ffd700;background-color:#ffd700;max-width: 100%}.PLUGIN-list-item {display: flex; align-items: center; padding: 0.6em 1.2em; margin: 0.4em 1em; background: #222222; border-radius: 0.4em; border-bottom: 2px solid #333333; cursor: pointer; transition: background 0.3s, transform 0.2s;}.PLUGIN-list-item.focus {background: #333333; color: #ffffff; transform: scale(1.01); border-bottom: 2px solid #ffd700; box-shadow: 0 5px 15px rgba(0,0,0,0.5);}.PLUGIN-list-item-logo-box {width: 65px !important; min-width: 65px !important; height: 45px !important; display: flex !important; justify-content: center; align-items: center; margin-right: 15px; flex-shrink: 0; background: rgba(0,0,0,0.3); border-radius: 4px; overflow: hidden;}.PLUGIN-list-item-logo-box img {width: 100%; height: 100%; object-fit: contain;}.PLUGIN-list-item-body {flex-grow: 1; display: flex; flex-direction: column; overflow: hidden;}.PLUGIN-list-item-title {font-size: 1.3em; font-weight: 500; color: #ffffff; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-bottom: 0.2em;}.PLUGIN-list-item-epg-box {display: none; flex-direction: column; width: 100%;}.PLUGIN-list-item-epg-text {font-size: 0.9em; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; opacity: 0.7; color: #bbbbbb;}.PLUGIN-list-item-progress {height: 3px; background: #444444; width: 100%; margin-top: 6px; border-radius: 2px; overflow: hidden;}.PLUGIN-list-item-progress > div {height: 100%; background: #ffd700; width: 0%;}.PLUGIN-list-item.focus .PLUGIN-list-item-progress > div {background: #ffd700;}.PLUGIN-list-item-icons {display: flex; align-items: center; margin-left: 10px;}.PLUGIN-list-item-icons .icon--book, .PLUGIN-list-item-icons .icon--timeshift {width: 24px; height: 24px; margin-left: 10px; opacity: 0.7;}</style>'.replace(/PLUGIN/g, plugin.component));
...
function pluginPage(object) {
    ...
    var html = $('<div></div>');
    var body = $('<div class="' + plugin.component + ' category-full" style="display: flex; flex-direction: column;"></div>');
	...
	if (Object.keys(catalog).length) {
		...
		while (i < l.length) {
			...
			var logoCandidates = [];
			var pushLogo = function (u) {
			    if (u && logoCandidates.indexOf(u) === -1) logoCandidates.push(u);
			};
			if (channel['tvg-logo']) {
			    if (/^https:\/\//i.test(channel['tvg-logo'])) {
				pushLogo(channel['tvg-logo']);
			    }
			    // http:// tvg-logo sources are skipped here: mixed-content blocks them,
			    // and forcing https just hangs for ~20-30s on hosts without TLS (e.g. epg.one).
			    // We rely on the iptv-org DB below instead.
			}
			if (logoMap) {
			    var logoKey = chShortName(channel['Title']);
			    pushLogo(logoMap[logoKey]);
			    pushLogo(logoMap[trName(logoKey)]);
			}
			// NOTE: epg.it999.ru does not resolve (dead domain) and epg.rootu.top/picon/
			// always 404s - both removed, they never produced a working logo and only
			// wasted a request per channel. Only tvg-logo (https) and the iptv-org DB remain.
			channel['tvg-logo'] = logoCandidates[0] || '';
			channel['_logoList'] = logoCandidates;
	...
	if (epg.length) {
		...
		if (cIdEl !== cId) {
            var timeString = toLocaleTimeString(e[0] * 60000) + ' - ' + toLocaleTimeString((e[0] + e[1]) * 60000);
            var epgText = timeString + ' • ' + (e[2] || '');
		    
		    epgEl.data('cId', cId);
		    epgEl.data('progress', p);
		    epgEl.find('.js-epgTitle').text(epgText);
		    epgEl.find('.js-epgProgress > div').css('width', p + '%');
		    epgEl.css('display', 'flex'); 
	...
	if (epgIdCurrent === epgId) {
		...
		if (epg.length > i) {
		    ...
		    for (; i < iEnd; i++) {
			e = epg[i];
			var itemAfter = epgItemTeplate.clone();
			itemAfter.find('.js-epgTime').text(toLocaleTimeString(e[0] * 60000));
			itemAfter.find('.js-epgTitle').text(e[2]);
			list.append(itemAfter);
		    }
		    epgAfter.show();
		} else {
		    epgAfter.hide();
	...
	var bulkFn = bulkWrapper(function (channel) {
	    var chI = chIndex++;
        var item = $('<div>', {class: ('PLUGIN-list-item selector').replace(/PLUGIN/g, plugin.component)});
        
        // Гарантоване створення контейнера логотипу з фіксованими інлайн-стилями
        var logoBox = $('<div>', {
            class: ('PLUGIN-list-item-logo-box').replace(/PLUGIN/g, plugin.component),
            style: 'width: 65px; min-width: 65px; height: 45px; display: flex; justify-content: center; align-items: center; margin-right: 15px; flex-shrink: 0; overflow: hidden;'
        });

        if (channel['tvg-logo']) {
            var logoCands = channel['_logoList'] && channel['_logoList'].length ? channel['_logoList'] : [channel['tvg-logo']];
            var logoIdx = 0;
            var img = $('<img>', {
                src: logoCands[0],
            ...
            img.on('error', function() {
                console.warn('[max_iptv_list] logo failed: "' + channel.Title + '" -> ' + logoCands[logoIdx]);
                logoIdx++;
                if (logoIdx < logoCands.length) {
                    $(this).attr('src', logoCands[logoIdx]);
                } else {
                    console.warn('[max_iptv_list] no working logo for: "' + channel.Title + '" (tried ' + logoCands.length + ')');
                    $(this).remove();
                }
            });
            logoBox.append(img);
        ...
        var bodyBox = $('<div>', {class: ('PLUGIN-list-item-body').replace(/PLUGIN/g, plugin.component)});
        var titleBox = $('<div>', {class: ('PLUGIN-list-item-title').replace(/PLUGIN/g, plugin.component)}).text(channel.Title);
        var epgBox = $('<div>', {class: ('PLUGIN-list-item-epg-box card__age').replace(/PLUGIN/g, plugin.component)});
        var epgTitle = $('<div>', {class: ('PLUGIN-list-item-epg-text js-epgTitle').replace(/PLUGIN/g, plugin.component)});
        var epgProgress = $('<div>', {class: ('PLUGIN-list-item-progress js-epgProgress').replace(/PLUGIN/g, plugin.component)});
        epgProgress.append($('<div>')); 
        epgBox.append(epgTitle).append(epgProgress);
        bodyBox.append(titleBox).append(epgBox);
        item.append(bodyBox);
        
        var iconsBox = $('<div>', {class: ('PLUGIN-list-item-icons').replace(/PLUGIN/g, plugin.component)});
        item.append(iconsBox);

	    var favIcon = $('<div>', {
            class: 'icon--book hide',
            style: 'background-image:url(https://vi.rootu.top/img/icon/book.svg); background-size: cover;'
        });
	    iconsBox.append(favIcon);
	    var tvgDay = parseInt(
	    ...
	    if (parseInt('catchup-enable' in channel ? channel['catchup-enable'] : tvgDay) > 0) {
            var shiftIcon = $('<div>', {
                class: 'icon--timeshift',
                style: 'background-image:url(https://epg.rootu.top/img/icon/timeshift.svg); background-size: cover;'
            });
		iconsBox.append(shiftIcon);
		if (tvgDay === 0) tvgDay = 1;
	    } else {
		tvgDay = 0;
	    }
	    if (object.currentGroup !== '' && favorite.indexOf(favID(channel.Title)) !== -1) {
		favIcon.removeClass('hide');
	    }
	    item.on('hover:focus hover:hover touchstart', function (event) {
		if (event.type && event.type !== 'touchstart' && event.type !== 'hover:hover') scroll.update(item, !true);
		last = item[0];
		...
		Lampa.Select.show({
			...
			if (!!sel.archive) {
				...
				for (var i=tvgData.length - 1; i >= 0; i--) {
					...
					archiveMenu.unshift(e_item);
					playlistMenu.push(e_item);
					playlist.push(e_item);
				    }
				}
				if (prevDate) {
				    archiveMenu.unshift({
			    ...
			    setStorage('epg', epg);
			    var scroll = item.parents(".scroll");
			    if (epg) {
	    ...
	    body.append(item);
	    if (!!channel['epgId']) {
		item.attr('data-epg-id', channel['epgId']);
		epgRender(channel['epgId']);
	    }
	},
	{
	...
	Lampa.Template.add(plugin.component + '_button_category', "<div class=\"full-start__button selector view--category\"><svg style=\"enable-background:new 0 0 512 512;\" version=\"1.1\" viewBox=\"0 0 24 24\" xml:space=\"preserve\" xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\"><g id=\"info\"/><g id=\"icons\"><g id=\"menu\"><path d=\"M20,10H4c-1.1,0-2,0.9-2,2c0,1.1,0.9,2,2,2h16c1.1,0,2-0.9,2-2C22,10.9,21.1,10,20,10z\" fill=\"currentColor\"/><path d=\"M4,8h12c1.1,0,2-0.9,2-2c0-1.1-0.9-2-2-2H4C2.9,4,2,4.9,2,6C2,7.1,2.9,8,4,8z\" fill=\"currentColor\"/><path d=\"M16,16H4c-1.1,0-2,0.9-2,2c0,1.1,0.9,2,2,2h12c1.1,0,2-0.9,2-2C18,16.9,17.1,16,16,16z\" fill=\"currentColor\"/></g></g></svg><span>" + langGet('categories') + "</span>\n	</div>");
...
langAdd('default_playlist', {ru: 'https://gitlab.com/iptv135435/iptvshared/raw/main/IPTV_SHARED.m3u',uk: 'https://gitlab.com/iptv135435/iptvshared/raw/main/IPTV_SHARED.m3u',be: 'https://gitlab.com/iptv135435/iptvshared/raw/main/IPTV_SHARED.m3u',en: 'https://gitlab.com/iptv135435/iptvshared/raw/main/IPTV_SHARED.m3u',zh: 'https://gitlab.com/iptv135435/iptvshared/raw/main/IPTV_SHARED.m3u'});
langAdd('default_playlist_cat', {ru: 'Эфирные',uk: 'Эфирные',be: 'Эфирные',en: 'Эфирные',zh: 'Эфирные'});
...
langAdd('favorites_del', {ru: 'Удалить из избранного',uk: 'Видалити з вибраного',be: 'Выдаліць з абранага',en: 'Remove from favorites',zh: '从收藏夹中删除'});
